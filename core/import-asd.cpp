 ...
 */
extern "C" unsigned char *build_dc_data(int model, dc_family_t dc_fam, unsigned char *input, int max, int *out_size)
{
...
{
	unsigned char *tmp = (unsigned char *)in.data();
	unsigned char *runner = tmp;
	const unsigned char *maxptr = tmp + in.size();

	while (runner < maxptr) {
		if (memcmp(runner, seq, s))
			runner += 1;
		else
			break;
	}
...
std::string asd_to_string(const std::string &input, std::string &output)
{
	...
	tmp += 4;
	while (i < size) {
		unsigned char c = (tmp[i + 1] << 4) + tmp[i];
		if (c <= 0x7F) {
			buffer[j] = c;
		}
		else {
 ...
 */
static void asd_build_dive_site(const char *instring, const char *coords, struct divelog *log, struct dive_site **asd_site)
{
...
bool is_smtk(const std::string &p)
{
	...
	std::size_t pos = uchar_find(p, smart, 10);
	std::size_t pos1 = p.find("CLogEntry");
	return pos < pos1;
}
...
static int asd_dive_parser(const std::string &input, struct dive *asd_dive, struct divelog *log)//, const device_table &devices)
{
	...
	dc_family_t dc_fam;
	long dc_serial;
	const unsigned char *ptr, str_seq[] = {0xff, 0xfe, 0xff}, dc_profile_begin[4] = {0x01, 0x00, 0x00, 0xFF};
	unsigned char *ptr1, *dc_data;
	char *d_coords = NULL;
	device_data_t *devdata = (device_data_t *)calloc(1, sizeof(device_data_t));
	asd_dive->dcs[0].serial = (char *)calloc(64, 1);
	weightsystem_t ws;
	std::string tmp, d_locat, d_point, notes, viz, w_type, w_surf, weather, equipment;
	std::size_t size;

	//input should point to dc model integer
	ptr = (unsigned char *)input.data();
	dc_model = ptr[0];
	ptr[1] > 0x00 ? dc_fam = DC_FAMILY_UWATEC_ALADIN : dc_fam = DC_FAMILY_UWATEC_SMART;
	tmp = input.substr(8);

	ptr = (unsigned char *)tmp.data();
	...
	if (!dc_model) {	// this would be the manual dive case (coming from SmartTrak)
		...
		ptr1 = (unsigned char *)tmp.data();
		size = uchar_find(tmp, str_seq, 3); // size of DC data
		dc_data = build_dc_data(dc_model, dc_fam, ptr1, size, &s);
		if (!dc_data)
			goto bailout;

		rc = libdc_buffer_parser(asd_dive, devdata, dc_data, s);
		if (rc != DC_STATUS_SUCCESS) {
			free(dc_data);
			goto bailout;
		}
	...
	if (!d_point.empty())
		d_locat += ", " + d_point;
	d_coords = (char *)(asd_to_string(tmp, tmp)).c_str();
	if (tmp.empty())
		goto buffail;

	asd_build_dive_site((char *)d_locat.c_str(), d_coords, log, &asd_dive->dive_site);
	// next two bytes are the tank volume (mililiters) and two following are
	// unknown (always zero, may be a 2º tank).
	ptr = (unsigned char *)tmp.data();
	asd_dive->get_or_create_cylinder(0)->type.size.mliter = (ptr[1] << 8) + ptr[0];
	tmp = tmp.substr(4);
	asd_dive->get_cylinder(0)->type.description = (char *)asd_to_string(tmp, tmp).c_str();
	if (tmp.empty())
		goto buffail;

	asd_dive->suit = (char *)asd_to_string(tmp, tmp).c_str();
	if (tmp.empty())
		goto buffail;

	// next two bytes are the weight in gr. Two following are zeroed.
	ptr = (unsigned char *)tmp.data();
	ws = { { .grams = (ptr[1] << 8) + ptr[0] }, translate("gettextFromC", "unknown"), false };
	...
	if (!weather.empty())
		taglist_add_tag(asd_dive->tags, weather.c_str());
	if (tmp.empty())
		goto buffail;
	...
	if (!w_surf.empty())
		taglist_add_tag(asd_dive->tags, w_surf.c_str());
	if (tmp.empty())
		goto buffail;
	...
	if (!w_type.empty())
		taglist_add_tag(asd_dive->tags, w_type.c_str());
	if (tmp.empty())
		goto buffail;
	...
	if (!viz.empty())
		taglist_add_tag(asd_dive->tags, viz.c_str());
	if (tmp.empty())
		goto buffail;
	...
	if (j > 0) {
		std::string out;
		tmp = asd_build_others(tmp, j, out);
		asd_dive->buddy = out.c_str();
	}
	if (tmp.empty())
		goto buffail;
...
bailout:						// give a pointer, we can parse other dives yet
	...
	//free((char *) devdata->model);
	free(devdata);
	//free((void *)asd_dive->dcs[0].serial);
	return -1;
...
{
	std::string runner;
	const char init_seq[] = {0x07, 0x00, 0x10, 0x00};
	std::string first_dive_seq = {"CLogEntry"};
	unsigned char dive_seq_lt[2] = {0x80, 0x01};
	unsigned char dive_seq_smtk[2] = {0x01, 0x80};
	...
	while (pos < std::string::npos) {
		...
		pos = 0;
		rc = asd_dive_parser(runner, asd_dive.get(), log); //->sites, log->devices);
		switch (rc) {
		case 0: {
			log->dives.record_dive(std::move(asd_dive));
			//record_dive_to_table(asd_dive, log->dives);
			break;
		}
		...
		while (pos < 100) {
			pos = uchar_find(runner, dive_seq, 2);
			runner = runner.substr(pos + 2);
		}
	};
	log->dives.sort();
	return 0;
