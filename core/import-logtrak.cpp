...
static std::string lt_remove_quotes(const std::string &input)
{
	...
	if (input[0] == 0x27)
		tmp = input.substr(1, input.length() - 1);
	else
		tmp = input;
...
static std::string trim_quotes(const std::string &in) {
	...
	s = in.find("\'\'");
	if (s > 0 && s < in.length()) {
		std::string h = in.substr(0, s);
		std::string t = in.substr(s + 1);
		h += t;
		return trim_quotes(h);
	} else {
		return in;
	}
}
 ...
 * BTW, I don't know/haven't found an straight/easy way to convert
 * UCN strings to utf-8 in C++, so I'll keep the good ole pure C way.
 */
static std::string u_str_to_utf8(const std::string &in)
{
	char *t, *r = (char *)in.c_str();
	int c = 0;
	...
	r += 2;
	c = (ASCII_CHR_TO_BYTE(r[0]) << 12) + (ASCII_CHR_TO_BYTE(r[1]) << 8) + (ASCII_CHR_TO_BYTE(r[2]) << 4) + ASCII_CHR_TO_BYTE(r[3]);
...
/*
 * Recursively parse a string containing unicode chars in
...
static std::string to_utf8(const std::string &in)
{
	...
	std::size_t pos = in.find("\\u");
	/* No unicode chars case */
	if (pos >= std::string::npos)
		return in;

	std::string h = in.substr(0, pos);
	std::string u_str = u_str_to_utf8(in.substr(pos, 6));
	std::string t = in.substr(pos + 6);
	h += (u_str + t);
	return to_utf8(h);
}
...
static void lt_auxiliary_parser(const std::string &buffer)
{
	...
	while (pos < std::string::npos) {
		...
		if (ltable == "T_BOTTLE") {
			char *tankmat, *tankid, *eq_ref;
			int tankvol = 0, o2mix = 0, startp = 0, endp = 0, hemix = 0, tanknum = 0;
			sscanf(line.c_str(), "%m[0-9],%d,%d,%d,%d,%m[a-zA-Z0-9-_'],%*d,%*[0-9a-zA-Z'],%d,%*d,%d,%m[0-9],", &tankid, &tankvol, &o2mix, &startp, &endp, &tankmat, &hemix, &tanknum, &eq_ref);
			Lt_String tmp(tankmat);
			bottle_db.push_back({eq_ref, tankid, tankvol, tanknum, startp, endp, o2mix, hemix, tmp.string()});
			free(tankmat);
			free(tankid);
			free(eq_ref);
			...
			char *eq_ref;
			float weight_s = 0;
			sscanf(line.c_str(), "%m[0-9],%*[0-9a-zA-Z'],%*[0-9a-zA-Z' \\],%f", &eq_ref, &weight_s);
			lpos = line.find(",'");
			...
			sscanf(line.c_str(),"%f,%f,", &GPSx, &GPSy);
			lpos = line.find(",'");
			line = line.substr(lpos + 2);
			line = get_lt_string(line, comm);
			sscanf(line.c_str(), "%*m[a-zA-Z0-9],%m[0-9])", &locid);
...
static void lt_get_tank_info(char *eq_id, struct dive *ltd)
{
	...
	for (i = 0; i < bottle_db.size(); i++) {
		if (bottle_db[i].equipment_id == eqid) {
			tanknum = bottle_db[i].tanknum - 1;
			if (bottle_db[i].tank_vol)
				ltd->get_or_create_cylinder(tanknum)->type.size.mliter = bottle_db[i].tank_vol;
			// Always prefer data from DC over data from Logtrak
			if (bottle_db[i].startp && !ltd->get_or_create_cylinder(tanknum)->start.mbar)
				ltd->get_or_create_cylinder(tanknum)->start.mbar = bottle_db[i].startp * 1000;
			if (bottle_db[i].endp && !ltd->get_or_create_cylinder(tanknum)->end.mbar)
				ltd->get_or_create_cylinder(tanknum)->end.mbar = bottle_db[i].endp * 1000;
			if (bottle_db[i].o2_mix && !ltd->get_or_create_cylinder(tanknum)->gasmix.o2.permille)
				ltd->get_or_create_cylinder(tanknum)->gasmix.o2.permille = bottle_db[i].o2_mix * 10;
			if (bottle_db[i].he_mix && !ltd->get_or_create_cylinder(tanknum)->gasmix.he.permille)
				ltd->get_or_create_cylinder(tanknum)->gasmix.he.permille = bottle_db[i].he_mix * 10;
			if (!bottle_db[i].tankmat.empty())
				ltd->get_or_create_cylinder(tanknum)->type.description = bottle_db[i].tankmat.c_str();
			break;
		}
	}
}
 ...
 */
//static void lt_build_dive_site(const char *site_id, struct dive_site_table *ds_table, struct dive_site **lt_site)
static void lt_build_dive_site(const char *site_id, struct divelog *log, struct dive_site **lt_site)
{
	struct dive_site *site;
	std::string built_name;
	location_t gps_loc;
	int i = 0;
	...
	while (site_db[i].site_id != site_id)
		i++;
	/* LogTrak enforces location/site structure, but lazy user may set just one, resulting in
	 * the same name for both, location and site. Ensure we just use one in this case.
	 */
	if (!site_db[i].site_name.empty() && site_db[i].site_name != site_db[i].site_loc.loc_name)
		built_name = site_db[i].site_loc.loc_name + ", " + site_db[i].site_name;
	else
		built_name = site_db[i].site_loc.loc_name;

	/* build gps position */
	gps_loc = create_location(site_db[i].GPSx, site_db[i].GPSy);
	/* build the dive site structure */
	site = log->sites.get_by_name(built_name.c_str());
	if (!site) {
		if (!has_location(&gps_loc))
			site = log->sites.create(built_name.c_str());
		else
			site = log->sites.create(built_name.c_str(), gps_loc);
	}
	if (!site_db[i].comment.empty())
		site->notes = site_db[i].comment.c_str();
...
int logtrak_import(const std::string &mem, struct divelog *log)
{
	...
	while (pos < std::string::npos) {
		...
		devdata = (device_data_t *)calloc(1, sizeof(device_data_t));
		pos = tmpstr.find('(');
		tmpstr = tmpstr.substr(pos + 1);
		/*Read up to time zone. Discard this one.*/
		sscanf(tmpstr.c_str(), "%m[0-9],'%m[a-zA-Z0-9]',%lf,'%m[a-zA-Z0-9]',%d,%d,]",\
		...
		if (db_version != "1.3.5")
			rc = sscanf(tmpstr.c_str(),"%m[0-9A-Z],%m[0-9A-Z],%*m[-0-9A-Z],%m[0-9A-Z],%m[0-9A-Z],%m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%d,%*m[-0-9A-Z],%lf,%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%m[0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%m[0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%m[0-9A-Z],%m[0-9A-Z],%m[0-9A-Z]",&ltd_ref_eq, &ltd_ref_site, &ltd_dc, &ltd_dc_id, &ltd_dive, &ltd_max_depth, &ltd_mintemp, &ltd_airtemp, &ltd_dc_soft, &ltd_gf_low, &ltd_gf_high, &ltd_log_id);
		else
			rc = sscanf(tmpstr.c_str(),"%m[0-9A-Z],%m[0-9A-Z],%*m[-0-9A-Z],%m[0-9A-Z],%m[0-9A-Z],%m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%d,%*m[-0-9A-Z],%lf,%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%m[0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[-0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%*m[-0-9A-Z],%*m[-0-9A-Z],%m[0-9A-Z],%*m[0-9A-Za-z'],%*m[0-9A-Za-z'],%m[0-9A-Z],%m[0-9A-Z],%m[0-9A-Z],",&ltd_ref_eq, &ltd_ref_site, &ltd_dc, &ltd_dc_id, &ltd_dive, &ltd_max_depth, &ltd_mintemp, &ltd_airtemp, &ltd_log_id, &ltd_dc_soft, &ltd_gf_low, &ltd_gf_high);
		if (rc < 12) {
		...
		if (!d.empty() && !is_null(d)) {
			...
			if (!lt_convert_profile((unsigned char *)d.data(), prf_buffer))
			...
			if (devdata->descriptor) {
				...
				libdc_buffer_parser(lt_dive.get(), devdata, prf_buffer, prf_size);
		...
		if (!empty_string(ltd_ref_eq)) {
			...
			while (equipment_db[i].equipment_id != eq_ref)
				i++;
			lt_dive->suit = equipment_db[i].suit.c_str();
		...
		add_extra_data(&lt_dive->dcs[0], "LogTrak Version", db_version.c_str());
		...
		//record_dive_to_table(lt_dive, log->dives);
		runner = runner.substr(pos + 1);
		pos = runner.find("INSERT INTO T_DIVE ");
		runner = runner.substr(pos + 1);
	}
